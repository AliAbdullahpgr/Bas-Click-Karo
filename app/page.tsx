...
import { characters, chaiGreetings, chaiReplyVariants, type Character } from './conversations';
import MohallaGossip from './MohallaGossip';
...
export default function Home() {
  ...
  const chaiCount = useRef(0);
  const chaiVisits = useRef(0);
  const chaiReplyCounts = useRef<number[]>([0, 0, 0, 0, 0]);
  ...
  const [cricketSeconds, setCricketSeconds] = useState(12);
  const cricketDeadline = useRef(0);
  const cricketHits = useRef(0);
  const cricketStartButton = useRef<HTMLButtonElement | null>(null);
  const [gossipOpen, setGossipOpen] = useState(false);
  const [musicOpen, setMusicOpen] = useState(false);
  const [catMessage, setCatMessage] = useState('');
  const catReplies = useRef(0);
  ...
  const playEffect = useSoundEffects(muted, dark);
  ...
  useEffect(() => {
    const visitors: Character[] = ['aunty', 'driver', 'chai', 'uncle', 'gossip', 'cat'];
    let visitor = 0;
  ...
  function pourTea(){chaiCount.current+=1;setSteam(true);sound('chai');later(()=>setSteam(false),2600);}
  function tea(){
    count();
    const greeting=chaiGreetings[chaiVisits.current++ % chaiGreetings.length];
    openChat('chai',greeting);
  }
  function sipTea(){
    count();pourTea();
    const line=chaiCount.current===5?'Bhai bas karo, acidity ho jayegi. Paanch cup ho gaye!':['Ahh, karak chai. Ab batao, din kaisa ja raha hai?','Garam hai ustad, aahista. Biscuit paas rakhe hain.','Cup khaali? Ek aur chahiye toh keh dena.'][(chaiCount.current-1)%3];
    openChat('chai',line);
  }
  function ride(){playEffect('engine');setDrive(true);setRideId(n=>n+1);if(rideTimer.current)clearTimeout(rideTimer.current);rideTimer.current=setTimeout(()=>setDrive(false),4400);}
  ...
  function reply(who: Character, index: number){
    const [question,baseResponse]=characters[who].replies[index];
    let response=baseResponse;
    if(who==='chai'){
      const variants=chaiReplyVariants[index];
      response=variants[chaiReplyCounts.current[index]++ % variants.length];
      if([0,1,4].includes(index)){
        pourTea();
        if(chaiCount.current===5)response+=' Bhai bas karo, acidity ho jayegi. Paanch cup ho gaye!';
      }
    }
    count();setChat(who);setGossipOpen(false);setAunty(false);
    setMessages(current=>[...(chat===who?current:[{mine:false,text:who==='uncle'?uncleQuestion:characters[who].greeting}]),{mine:true,text:question},{mine:false,text:response}].slice(-8));
    if(who==='driver')ride();
  ...
  function stopCricket(showScore=true){
    setBat(false);
    if(showScore)say('Over khatam! '+cricketHits.current+' runs. Ab chai break. 🏏');
  ...
  useEffect(()=>{
    if(!bat)return;
    const finish=()=>{setBat(false);say('Over khatam! '+cricketHits.current+' runs. Ab chai break. 🏏');};
    const check=()=>{const left=Math.max(0,Math.ceil((cricketDeadline.current-Date.now())/1000));setCricketSeconds(left);};
    const interval=setInterval(check,200);
    const timeout=setTimeout(finish,Math.max(0,cricketDeadline.current-Date.now()));
    const onKey=(event:KeyboardEvent)=>{if(event.key==='Escape'){setBat(false);setChaos(false);say('Bat wapas rakh diya.');}};
    document.addEventListener('keydown',onKey);
    return()=>{clearInterval(interval);clearTimeout(timeout);document.removeEventListener('keydown',onKey);};
  },[bat]);
  function honk(){count();setHorns(h=>h+1);sound();say(horns>=3?'Pakistan mein horn bajana zaroori nahi hota. …PON PON!':'PON PON! 📣');}
  function askUncle(){playEffect('laugh');count(); if(uncle>=3){setBossHealth(3);setBoss(true);}else{setUncleQuestion(uncleLines[uncle]);setUncleReaction('');setUncleAngry(false);openChat('uncle',uncleLines[uncle]);setUncle(u=>u+1);}}
  ...
  function petCat(){
    count();playEffect('meow');openChat('cat');setCatHop(true);later(()=>setCatHop(false),1100);
  }
  function messageCat(){
    const text=catMessage.trim();if(!text)return;
    const answer=['Meow meow.', 'Meeeow. Meow?', 'Meow meow meow.', 'Mrrrow. Meow.'][catReplies.current++ % 4];
  ...
  return <main className={bat?'bat-mode':''}>
    ...
    <h1 className="sr-only">Bas Click Karo — Internet pe aaye ho. Ab kuch kaam ka mat karna.</h1><div className="scene-scroll" tabIndex={0} aria-label="Explore the street; scroll sideways on smaller screens"><section className={'scene '+(chaos?'chaos':'')} aria-label="Interactive Pakistani street">
      ...
      <button className={"hotspot wedding "+(music.dholPlaying?"wedding-playing":"")} aria-label="Shaadi dhol" aria-pressed={music.dholPlaying} onClick={shaadi}><span className="scene-label">{music.dholPlaying?"🥁 Dhol band karo":"🥁 Shaadi ka dhol"}</span>{music.dholPlaying&&<span className="dhol-notes" aria-hidden="true">♫ ♪ ♫</span>}</button>
      <button className="hotspot gossip" aria-label="Open Mohalla Gossip" onClick={()=>{count();openChat("gossip");}}><span className="scene-label">🌿 Mohalla gossip</span></button>
      <button className={"hotspot music-box "+(music.playing?"music-box-playing":"")} aria-label="Open Good Music player" onClick={openPlayer}><span className="scene-label">♫ Music player</span></button>
      ...
      {(chaos||bat)&&<div className="emoji-field">{['🏏','🥭','🏆','☕','🩴','💸','🍗','❤️','🎉','🛺','🥭','🩴'].map((emoji,i)=><button key={i} aria-label={'Hit '+emoji} style={{left:(8+i*7.4)+'%',animationDelay:(i*.19)+'s',animationDuration:(3+i%3)+'s'}} onClick={e=>{if(!bat)return;cricketHits.current+=1;setHits(cricketHits.current);sound('chai');e.currentTarget.style.visibility='hidden';const target=e.currentTarget;later(()=>target.style.visibility='visible',900);}}>{emoji}</button>)}</div>}
      {bat&&<div className="game-score" role="status">🏏 {hits} runs <span className="cricket-clock">{cricketSeconds}s</span><button onClick={()=>stopCricket()}>Stop ×</button><div className="cricket-timer"><i style={{width:(cricketSeconds/12*100)+"%"}}/></div></div>}
      ...
      <div className="scene-dock">{[['🎮','Games'],['☕','Chai break'],['📣','Horn bajao'],['⚡','Bijli check'],['🎲','Surprise me'],['🇵🇰','Pakistani test']].map(([icon,label],i)=><button key={label} onClick={()=>[()=>bat?stopCricket():startCricket(),tea,honk,blackout,redButton,()=>{setCorrect(0);setQuiz(0);setSelected(null);}][i]()}><span>{icon}</span>{label}</button>)}</div>
    </section></div>
    ...
    {dark&&<div className="blackout" role="alert"><span>💡</span><p>WELCOME TO PAKISTAN</p><h1>LOAD<br/>SHEDDING.</h1><p className="pole-reaction">“Mar jaata abhi!<br/>Shukar hai light nahi hai.”</p><div className="power-progress"/><small>Bijli aa rahi hai… probably.</small></div>}
